...
import { DeleteIcon, EditIcon, DuplicateIcon, SendToBackIcon, DpadUpIcon, DpadDownIcon, DpadLeftIcon, DpadRightIcon, PlayIcon, PauseIcon, ResetIcon, CopyIcon, PasteIcon, DownloadIcon, FileIcon } from './Icons';
...
const DraggableItem: React.FC<DraggableItemProps> = ({ item, onUpdate, onDelete, onDuplicate, onEdit, onSendToBack, onSaveToInventory, boardRef, zoom, snapToGrid, gridSize, isMobileMode, isSelected, onSelect, selectedItemIds }) => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    if (isSelected && selectedItemIds.length > 1) {
      selectedItemIds.forEach(id => {
        ...
        const el = document.querySelector(`[data-item-id="${id}"]`) as HTMLElement;
        if (el) {
          const originalX = parseFloat(el.getAttribute('data-original-x') || '0');
          const originalY = parseFloat(el.getAttribute('data-original-y') || '0');
          el.style.left = `${originalX + dx}px`;
          el.style.top = `${originalY + dy}px`;
        }
      });
    }
  }, [isDragging, boardRef, item.width, item.height, item.x, item.y, zoom, snapToGrid, gridSize, isSelected, selectedItemIds]);
